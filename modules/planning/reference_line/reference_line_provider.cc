...
bool ReferenceLineProvider::GetReferenceLines(
  ...
  if (FLAGS_enable_reference_line_provider_thread) {
    ...
    double start_time = Clock::NowInSeconds();
    if (CreateReferenceLine(reference_lines, segments)) { // 平滑是在这里做的？
      UpdateReferenceLine(*reference_lines, *segments);
      double end_time = Clock::NowInSeconds();
      last_calculation_time_ = end_time - start_time;
      return true;
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
    std::list<ReferenceLine> *reference_lines,
    std::list<hdmap::RouteSegments> *segments) {
      // 将车道线保存到两个参数里面。特别的，找到了target车道方向上，离本车道最近的车道，设置：
      //   egment.SetIsNeighborSegment(true);
  ...
  using LaneIdPair = std::pair<std::string, uint32_t>; //first: 参考线id, second: 优先级
  ...
  for (const auto &path_pair : relative_map_->navigation_path()) {
    ...
    if (adc_lane_id != lane_id && priority < adc_lane_priority) {
      high_priority_lane_pairs.emplace_back(lane_id, priority); // 储存优先级比当前车道高的车道。
    }
  }
  // get the target lane
  ...
  std::string nearest_neighbor_lane_id; // target车道的方向的离本车道最近的车道。
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(
    const ReferenceLine &reference_line, double s) const {
      // 首先求车道宽度，然后根据车宽收缩，如果两边是curb，也收缩。
      // 返回的anchor point： 位置是道路中间（如果左边curb右边不curb的话也不是道路中间，总之是收缩后的道路中间，收缩不一定对称）
      // lateral_bound是收缩后道路宽度的一半。
  AnchorPoint anchor;
