...
common::Status SpeedDecider::Execute(Frame* frame,
                                     ReferenceLineInfo* reference_line_info) {
                                      //  根据搜索得到的速度曲线对障碍物做决策。包括跟车、避让、超车等。
...
Status SpeedDecider::MakeObjectDecision(
    const SpeedData& speed_profile, PathDecision* const path_decision) const {
      // speed_profile是上一个task在st图上进行搜索的结果。
      // 这个函数根据搜索的速度曲线，对障碍物进行决策。对于一个障碍物的st边界，先算出本车速度曲线在
      // 这个边界的上面还是下面还是冲突了，如果在下面，可能做出跟车或避让的决策。如果在上面，做出超车的巨册，
      // 如果有冲突，拉倒吧。
  ...
  for (const auto* obstacle : path_decision->obstacles().Items()) {
    ...
    switch (location) {
      case BELOW:
        ...
        } else if (CheckIsFollow(*obstacle, boundary)) {  // 根据障碍物的st图判断是否应该跟车。
...
// reasonable deceleration
        ...
        } else { // CheckIsFollow函数给出不是跟车的结果，则避让。
          // YIELD decision
...
bool SpeedDecider::CheckIsFollow(const Obstacle& obstacle,
  ...
  // 根据障碍物的st图判断是否应该跟车。是在确定st图上，本车的dp搜索结果在该障碍物的下方之后，才调用这个函数。
