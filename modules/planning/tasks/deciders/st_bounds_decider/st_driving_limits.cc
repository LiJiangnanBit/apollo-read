...
std::pair<double, double> STDrivingLimits::GetVehicleDynamicsLimits(
  ...
  double dec_time = lower_v0_ / max_dec_; // 最快多久能从lower_v0减速到0.
  if (t - lower_t0_ < dec_time) {
    dynamic_limits.first = // 如果以最大减速度减速，到t时刻的距离。。
        lower_s0_ + (lower_v0_ - max_dec_ * (t - lower_t0_) + lower_v0_) *
                        (t - lower_t0_) * 0.5;
  } else {
    dynamic_limits.first = lower_s0_ + (lower_v0_ * dec_time) * 0.5; // lower_s0_ + 减速到0的距离
  }

  // Process upper bound: (constant acceleration)
  double acc_time = (max_v_ - upper_v0_) / max_acc_; // 从upper_v0加速到最大速度用的时间
...
void STDrivingLimits::UpdateBlockingInfo(const double t, const double lower_s,
  ...
  if (curr_bounds.first < lower_s) { // 如果根据车辆加减速能力计算出的s下界小于当前t做出的决策的s下界
