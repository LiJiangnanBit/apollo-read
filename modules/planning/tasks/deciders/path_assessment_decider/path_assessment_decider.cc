...
Status PathAssessmentDecider::Process(
  ...
  for (size_t i = 0; i != valid_path_data.size(); ++i) {
    ...
    // 赋值curr_path_data的path_point_decision_guide_，包含了这段路的每个点是IN_LANE OUT_OF_LANE
  ...
  *(reference_line_info->mutable_path_data()) = valid_path_data.front(); // 注意这个赋值
...
void PathAssessmentDecider::SetPathInfo(
  ...
  if (reference_line_info.IsChangeLanePath()) {
    ...
    // 对于换道，将换道过程中的那一段设为OUT_ON_FORWARD_LANE；
    // 正常是IN_LANE    
    // 这个函数只更改path_decision的第二项，即标签。
    SetPathPointType(reference_line_info, *path_data, true, &path_decision);
  } else {
    // 对于借道，将中间那段不在本车道的标为OUT_ON_FORWARD_LANE或OUT_ON_REVERSE_LANE。
    // 正常是IN_LANE
    ...
    if (reference_line_info.reference_line().GetLaneWidth(
      ...
      if (is_lane_change_path) {
        ...
        if (ego_sl_boundary.start_l() > lane_left_width ||
            ego_sl_boundary.end_l() < -lane_right_width) { // 意思是车不在该车道内。该车道是换道车道，故车还没有开始换道。
            // start_l是右边，end_l是左边
