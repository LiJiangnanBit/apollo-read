...
Status SpeedBoundsDecider::Process(
    Frame *const frame, ReferenceLineInfo *const reference_line_info) {
      // 对参考线：利用LoadData函数给一些成员赋值，比较重要的有障碍物们的st边界，和沿着规划的路径的速度限制。
      // 对于障碍物，若没有纵向决策，那么计算其st边界；如果有纵向决策，那么对其st边界设置相关属性，并调整st边界。
      // lane_follow_stage里面两次调用这个task，区别可能就在这儿？
...
// 给每个障碍物的path_st_boundary赋值

// ComputeSTBoundary()根据纵向决策类型，给障碍物的path_st_boundary_的boundary_type_赋值；同时每个决策里面还有一个纵向距离s，比如跟车要保持的
      // 距离，给characteristic_length_赋值这个s。
      // 如果是yield，还要将上下界都扩展这个s距离。
...
// 成员：
  ...
  // 这应该是st_boundary_decider的结果。st_blundary_decider是对它的st_drivable_boundary_成员赋值。
...
// 这就是一个赋值。注意不要认为st_graph_data是一个局部变量，出去就没了；它是一个reference_line_info_的成员！
// st_blundary_decider是对它的st_drivable_boundary_成员赋值。这里对他的其他成员赋值
