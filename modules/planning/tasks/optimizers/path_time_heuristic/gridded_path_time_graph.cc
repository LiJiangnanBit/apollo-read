...
// 初始化speed_limit_by_index_，即每个s对应的速度上限。
...
// 计算每个st位置的cost，包含obstacle cost， spatial cost和速度 加速度 加加速度cost
...
Status GriddedPathTimeGraph::InitCostTable() {
  // 初始化table，包括计算时间和s的维度、间隔，和初始化cost table等。
  ...
  // s分为两部分，前一部分用较小的分辨率，后面用较大的分辨率。
  if (dense_dimension_s_ < 1) { // dense_dimension_s_是config里面读取的。
...
//   double total_cost_ = std::numeric_limits<double>::infinity();
  ...
  for (uint32_t i = 0; i < cost_table_.size(); ++i, curr_t += unit_t_) {
    auto& cost_table_i = cost_table_[i]; // 当前t
    double curr_s = 0.0;
    for (uint32_t j = 0; j < dense_dimension_s_; ++j, curr_s += dense_unit_s_) {
      cost_table_i[j].Init(i, j, STPoint(curr_s, curr_t)); // init就是一个赋值。
    }
...
Status GriddedPathTimeGraph::CalculateTotalCost() {
  ...
  for (size_t c = 0; c < cost_table_.size(); ++c) { // 循环t
...
// 就一行：return (total_s_ - point.point().s()) * config_.spatial_potential_penalty();
// 也就是s越大越好。
  ...
  const double cruise_speed = st_graph_data_.cruise_speed(); // 这个跟参考线有关。
  ...
  if (c == 1) {
    const double acc =
        2 * (cost_cr.point().s() / unit_t_ - init_point_.v()) / unit_t_; // 算最开始的加速度
  ...
  const double pre_lowest_s = // 能够到当前s的前一个t的最小s
      cost_cr.point().s() -
  ...
  const uint32_t r_pre_size = r - r_low + 1; // 等到本时刻的s的 前一个t时刻的s范围（index）
  ...
  for (uint32_t i = 0; i < r_pre_size; ++i) {
    ...
    if (pre_col[r_pre].GetOptimalSpeed() + curr_a * unit_t_ < -kDoubleEpsilon &&
        cost_cr.point().s() > min_s_consider_speed) { //  前面： const double min_s_consider_speed = dense_unit_s_ * dimension_t_;
      continue;
    }

    // st_graph_data_.st_boundaries包含了每个障碍物的st边界。
    if (CheckOverlapOnDpStGraph(st_graph_data_.st_boundaries(), cost_cr,
                                pre_col[r_pre])) {
      continue;
    }
