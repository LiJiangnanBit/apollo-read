        ...
        switch (boundary->boundary_type()) {
          ...
          case STBoundary::BoundaryType::FOLLOW:
            s_upper_bound =
                ...
                // EvaluateByTime（）就是对搜索得到的结果做一个线性插值，找到t时刻大概的s v a。
            ...
            s_soft_upper_bound =
                std::fmin(s_soft_upper_bound,
                          s_upper - FLAGS_follow_min_distance - // follow_min_distance默认是3m
                              std::min(7.0, FLAGS_follow_time_buffer * sp.v())); // 根据大概的速度设置一个缓冲区。
            break;
