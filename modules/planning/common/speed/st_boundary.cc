...
// 注意这两个循环，实际上是找到了ST图上占据的边界。按逆时针方向。
...
// 下面这个函数，给以下成员赋值：
// line_segments_: 将逆时针方向排列的边界点用线段连起来。
// 最大最小位置？？：
// min_x_ 
//   max_x_ 
//   min_y_ 
...
bool STBoundary::GetUnblockSRange(const double curr_time, double* s_upper,
  ...
  *s_lower = 0.0;
  ...
  size_t left = 0;
  size_t right = 0;
   //  找到t在points里面的index，然后left指向index-1， right指向index。
...
bool STBoundary::GetBoundarySRange(const double curr_time, double* s_upper,
  ...
  size_t left = 0;
  size_t right = 0;
  // lower_points是每个t对应的下界。找到第一个大于等于curr_time的元素对应的index，left=index-1, right=index。
  if (!GetIndexRange(lower_points_, curr_time, &left, &right)) {
    AERROR << "Fail to get index range.";
    return false;
  }
...
// 就是一个近似操作，upperpoints和lowerpoints都是离散的，这里用线性插值找到t时刻对应的上下s。
