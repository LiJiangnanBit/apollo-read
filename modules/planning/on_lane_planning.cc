...
Status OnLanePlanning::Init(const PlanningConfig& config) {
  ...
  planner_dispatcher_->Init(); // 注册public_road的规划器
  ...
  // dispatch planner
  planner_ = planner_dispatcher_->DispatchPlanner(); // 返回的是public_road_plannerd（动态类型）的unique_ptr<Planner>
  // 注意unique_ptr虽然一般情况下不能赋值，但是可以作为函数返回值赋值。cpp primer原书418
  if (!planner_) {
    return Status(
