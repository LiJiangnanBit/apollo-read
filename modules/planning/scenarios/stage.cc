...
Stage::Stage(const ScenarioConfig::StageConfig& config) : config_(config) {
  ...
  for (const auto& task_config : config_.task_config()) {
    config_map[task_config.task_type()] = &task_config; // config_map存储了本个stage里面，所有的tast_type及对应的tast_config
  }
  for (int i = 0; i < config_.task_type_size(); ++i) {
    ...
    if (iter == tasks_.end()) { // 如果tasks_里没有这个task_type，则实例化一个该类型的task
      auto ptr = TaskFactory::CreateTask(*config_map[task_type]);
      task_list_.push_back(ptr.get());
      tasks_[task_type] = std::move(ptr);
    } else {
      task_list_.push_back(iter->second.get()); // 如果tasks_里面已经有了这个task_type，说明已经实例化过了，那么就直接放入task_list_
    }
  } // 循环结束后，tasks_有本stage所有的task类型和task实例的对应；task_type里面依次包含所有的task。
    // 前者是不重复的，后者可以重复的。
}
