...
void ScenarioManager::ScenarioDispatch(const common::TrajectoryPoint& ego_point,
  ...
  if (scenario_type == default_scenario_type_) { // 除了当前正在紧急停车或parkandgo，其余都进这个判断。
    // check current_scenario (not switchable)
    switch (current_scenario_->scenario_type()) {
      case ScenarioConfig::LANE_FOLLOW:
      case ScenarioConfig::PULL_OVER:
        break; // 如果当前场景是正常行驶或靠边停车，跳出继续，scenario_type还是default。
      case ScenarioConfig::BARE_INTERSECTION_UNPROTECTED:
      case ScenarioConfig::EMERGENCY_PULL_OVER:
      case ScenarioConfig::PARK_AND_GO:
      case ScenarioConfig::STOP_SIGN_PROTECTED:
      ...
      case ScenarioConfig::YIELD_SIGN:
        ...
        if (current_scenario_->GetStatus() !=
            Scenario::ScenarioStatus::STATUS_DONE) {
          scenario_type = current_scenario_->scenario_type(); // 如果当前场景是以上一些的话，如果还没完成，scenario_type设置为当前场景。
        }
        break;
      default:
        break;
  ...
  // 到这里为止，scenario_type可能的情况：
  // 1. 在SelectPadMsgScenario被设置成紧急靠边停车，紧急停车或parkandgo
  // 2. 一些当前所在场景还没有完成，那就设置成这些场景。
  // 3. default, 即Lane follow。
  ...
  if (current_scenario_->scenario_type() != scenario_type) {
    current_scenario_ = CreateScenario(scenario_type); // 修改当前场景。
  }
}
